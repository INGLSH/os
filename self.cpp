...
// 逐位比较两个字符串（不区分大小写），相等返回0
static int stricmp_const(const char* a, const char* b)
{
	unsigned char ca, cb;
	while (*a && *b) {
		ca = (unsigned char) std::tolower((unsigned char)*a);
		cb = (unsigned char) std::tolower((unsigned char)*b);
...
// 只比较前n个字符
static int strnicmp_const(const char* a, const char* b, size_t n)
{
	unsigned char ca, cb;
	size_t i = 0;
	while (i < n && *a && *b) {
...
int ReplaceComd(int);			//replace命令处理函数

int ProcessPath(char*,char* &,int,int,char);	//处理路径名
...
int main()
{
	...
	//读入磁盘块Disk[ ]信息
	ffi.open("Disk2008.dat", ios::in | ios::binary);
	if (!ffi)
	{
	...
	ffi.open("UdTab2008.dat", ios::in | ios::binary);
	if (!ffi)
	{
...
void ExecComd(int k)		//执行命令
{
	...
	{
		if (stricmp_const(CmdTab[cid],comd[0])==0) //命令不区分大小写
			break;
	}
	...
	switch(cid)
	{
		 ...
		 case 26:FcComd(k);		//fc命令
				 break;
		 case 27:ReplaceComd(k);	//replace命令
				 break;
		 default:cout<<"\n命令错:"<<comd[0]<<endl;
	}
}
...
// replace <文件名> <目录名>：以“文件名”指定的文件，取代“目录名”指定目录中的同名文件
int ReplaceComd(int k)
{
	short s1,s2,sd,b,b0;
	short bneed,oldb;
	char attrib='\0',*FileName;
	char gFileName[PATH_LEN];
	FCB *fcbps,*fcbpd,*fcbpdir;
	...
	// 查找源文件
	s1=ProcessPath(comd[1],FileName,k,0,'\20');
	if (s1<1)
		return s1;
	s1=FindFCB(FileName,s1,attrib,fcbps);
	if (s1<0 || fcbps->Fattrib>='\20')
	{
		cout<<"\n源文件不存在或不是普通文件。\n";
		return -2;
	}
	...
	// 查找目标目录
	sd=FindPath(comd[2],'\020',1,fcbpdir);
	if (sd<1)
	{
		cout<<"\n目标目录不存在。\n";
		return sd;
	...
	// 在目标目录中查找同名文件
	s2=FindFCB(FileName,sd,attrib,fcbpd);
	if (s2<0)
	{
		cout<<"\n目标目录中没有同名文件，无法替换。\n";
		return -3;
	...
	if (Check_UOF(gFileName)<S)
	{
		cout<<"\n文件"<<gFileName<<"正在使用，不能替换。\n";
		return -5;
	}
	if (fcbpd->Fattrib&1)
	{
	...
	// 检查磁盘空间：释放目标文件后是否足够
	bneed=fcbps->Fsize/SIZE+(short)(fcbps->Fsize%SIZE>0);
	oldb=fcbpd->Fsize/SIZE+(short)(fcbpd->Fsize%SIZE>0);
	if (bneed>0 && FAT[0]+oldb<bneed)
	{
		cout<<"\n磁盘空间不足，替换失败。\n";
		return -7;
	}
	...
	// 释放原目标文件盘块
	if (fcbpd->Addr>0)
		releaseblock(fcbpd->Addr);

	// 复制源文件内容
	if (fcbps->Fsize==0)	// 源文件为空
	{
		fcbpd->Addr=0;
		fcbpd->Fsize=0;
		fcbpd->Fattrib=fcbps->Fattrib;
		cout<<"\n文件替换成功。\n";
		return 1;
	}
	...
	while (s1>0)
	{
		...
		if (b0==0)
			...
			FAT[b0]=b;
		memcpy(Disk[b],Disk[s1],SIZE);
		b0=b;
		s1=FAT[s1];
	}
	fcbpd->Fsize=fcbps->Fsize;
	fcbpd->Fattrib=fcbps->Fattrib;
	cout<<"\n文件替换成功。\n";
	return 1;
}
...
int FcComd(int k)
{
	...
	s1=ProcessPath(comd[1],FileName1,k,0,'\20');
	if (s1<1)			//路径错误
		return s1;		//失败，返回
	s1=FindFCB(FileName1,s1,attrib,fcbp1);		//取FileName的首块号(查其存在性)
	if (s1<0 || fcbp1->Fattrib>='\20')
	{
		cout<<"\n文件"<<FileName1<<"不存在或不是普通文件。\n";
		return -2;
	}

	s2=ProcessPath(comd[2],FileName2,k,0,'\20');
	if (s2<1)			//路径错误
		return s2;		//失败，返回
	s2=FindFCB(FileName2,s2,attrib,fcbp2);		//取FileName的首块号(查其存在性)
	if (s2<0 || fcbp2->Fattrib>='\20')
	{
		cout<<"\n文件"<<FileName2<<"不存在或不是普通文件。\n";
		return -3;
	}
...
int MdComd(int k)		//md命令处理函数
{
	...
	// 解析用户输入的目录名/路径，得到父目录盘块号s
	s=ProcessPath(comd[1],DirName,k,0,attrib);
	if (s<0)
		return s;		//失败，返回
	if (!IsName(DirName))		//若目录名字不符合规则
	{
		cout<<"\n命令中的新目录名错误。\n";
		return -1;
	}
...
int WriteComd(int k)		//write命令的处理函数
{
	...
	{
		if (strnicmp_const(comd[2],"app",3)==0)
			pos=uof[ii_uof].fsize+1;	//文件尾部添加模式(write <文件名> append)
		else if (strnicmp_const(comd[2],"ins",3)==0)
		{
		...
		else
		{
			...
			if (k==3)
			{
				if (strnicmp_const(comd[3],"ins",3)==0)
...
int AttribComd(int k)	//attrib命令的处理函数：修改文件或目录属性 
{
	...
	{
		for (j=0;j<6;j++)
			if (stricmp_const(comd[i],attr[j])==0)
			{
				if (j<3)
